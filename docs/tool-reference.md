...
Default and explicit `compact` modes register the compact description with the compact parameter schema: identical properties to the full schema, trimmed descriptions. The advertised file-defined subagent catalog in the system prompt is capped separately (16 agents, 4,096 bytes total, 160-byte summaries). See [`toolDescriptionMode`](configuration.md#tooldescriptionmode) for sizes and mode selection.
