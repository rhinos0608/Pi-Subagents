...
async function resumeAsyncRun(input: {
	...
	if (input.params.workflowParentRunId !== undefined && result.details.asyncDir) {
		...
		const childResult: SingleResult = {
			...
			...(completed.modelResolution ? { modelResolution: completed.modelResolution } : {}),
...
async function runAsyncPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details> | null> {
	...
	if (hasSingle) {
		...
		const asyncResult = executeAsyncSingle(id, compactOptional<Parameters<typeof executeAsyncSingle>[1]>({
			...
			modelResolutionSource: modelOverrideFromParent ? "parent-session" : params.model !== undefined ? "explicit-child" : a.model !== undefined ? "agent-config" : "default",
			modelResolutionRequested: modelOverrideFromParent ? undefined : params.model ?? a.model,
...
async function runSinglePath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	try {
		r = await runSync(ctx.cwd, agents, params.agent!, task, compactOptional<Parameters<typeof runSync>[4]>({
			...
			modelOverride,
			modelOverrideFromParent,
			modelResolutionSource: modelOverrideFromParent ? "parent-session" : params.model !== undefined ? "explicit-child" : agentConfig.model !== undefined ? "agent-config" : "default",
			modelResolutionRequested: modelOverrideFromParent ? undefined : params.model ?? agentConfig.model,
