...
- `checkpointBeforeDeadlineMs` — async single-agent runs only: asks the child to checkpoint-and-stop this many ms before the run deadline (finish current tool call, report state, start no new work); best-effort steer at the next tool boundary, ordinary deadline kill still applies
